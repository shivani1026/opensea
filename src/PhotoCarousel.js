  ...
  <Slider {...settings} className="h-1/3">
    {videos.map((video, index) => (
      <div key={index} className="flex justify-center items-center">
        <img src={video} alt={`Carousel ${index + 1}`} className="h-96 rounded-3xl mx-auto w-10/12 mt-5 mb-1 hover:-translate-x-1 hover:scale-105 transition-all duration-300" />
      </div>
    ))}
  </Slider>
  <Trending/>
